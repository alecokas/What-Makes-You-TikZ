...
\begin{tikzpicture}[scale=0.4, samples=280, domain=0:7*360]
	...
	% Zoomed in black-box
	\node[whiteSquareNode] (z1) at (6, -5.5) {\parbox{2cm}{\centering Acoustic \\ Model}};
	\node[whiteSquareNode] (z2) at (12, -5.5) {Lexicon};
	\node[whiteSquareNode] (z3) at (18, -5.5) {\parbox{2cm}{\centering Language \\ Model}};
	\node[whiteSquareNode] (z4) at (24.5, -5.5) {\includegraphics[width=.175\textwidth]{black-box-asr-detailed/mini-lattice.pdf}};
	\draw[stateTransition] (z1) to[out=0,in=180] node [midway, sloped, below] {} (z2);
	\draw[stateTransition] (z2) to[out=0,in=180] node [midway, sloped, below] {} (z3);
	\draw[stateTransition] (z3) to[out=0,in=180] node [midway, sloped, below] {} (z4);
    \draw[thick,dotted, rounded corners] ($(z1.north west)+(-0.5,0.5)$) rectangle ($(z4.south east)+(0.5,-0.5)$);
    % solid lines
    \draw[line] ($(h2.south west)$) to[out=190,in=10] node [midway, sloped, below] {} ($(z1.north west)+(-0.5,0.5)$);
    \draw[line] ($(h2.south east)$) to[out=-10,in=170] node [midway, sloped, below] {} ($(z4.north east)+(0.5,0.5)$);
